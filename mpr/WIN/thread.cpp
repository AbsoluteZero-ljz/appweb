...
MprThread::MprThread(int priority, char *name)
{
	osThreadId = 0;
#if !WINCE
    //  TODO -better to provide getpid stub
	pid = getpid();
#endif
	threadHandle = 0;
...
int MprThread::start()
{
	...
	lock();
#if WINCE
    h = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE) threadProcWrapper, (void*) this, 0, (LPDWORD) &threadId);
#else
	h = (HANDLE) _beginthreadex(NULL, 0, threadProcWrapper, (void*) this, 0, &threadId);
#endif
	if (h == NULL) {
		unlock();

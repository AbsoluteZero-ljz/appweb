///
///	@file 	WIN/os.cpp
/// @brief 	Windows support for the Embedthis Portable Runtime
...
int mprReadRegistry(char *key, char *name, char **buf, int max)
{
	...
	if (RegOpenKeyEx(top, (KEY_CAST*) key, 0, KEY_READ, &h) != ERROR_SUCCESS) {
		return MPR_ERR_CANT_ACCESS;
	}
	...
	//	Get the type
	// 
	if (RegQueryValueEx(h, (KEY_CAST*) name, 0, &type, 0, &size) != ERROR_SUCCESS) {
		RegCloseKey(h);
